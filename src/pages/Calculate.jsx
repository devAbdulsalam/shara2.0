...
const Calculate = () => {
  ...
  const calculateTotal = () => {
    ...
    // //Total Kg of waste
    setTotalKg(Number(metalWaste) + Number(plasticWaste) + Number(organicWaste) + Number(nonRMaterialWaste))
    if (total === 0){
      setAlert("visible")
      setTimeout(() => {
        setAlert('invisible')
      ...
      <div className="mt-14 py-8 mx-2 relative">
        <div className="flex flex-col bg-white rounded-lg shadow-lg overflow-hidden justify-around p-0 mx-auto min-w-sm lg:w-10/12">
              ...
              <div className="overflow-hidden">
                <table className="min-w-full">
                  <thead className="bg-gray-300 border-b">
                    <tr>
                      <th scope="col" className="text-sm md:text-xl font-medium text-gray-900 px-6 py-4 text-left">
                        Waste
                      </th>
                      <th scope="col" className="text-sm md:text-xl font-medium text-gray-900 px-6 py-4 text-left">
                        &#8358;/1kg
                      </th>
                      <th scope="col" className="text-sm md:text-xl font-medium text-gray-900 px-6 py-4 text-center">
                        Kg
                      </th>
                      <th scope="col" className="text-sm md:text-xl font-medium text-gray-900 px-6 py-4 text-center">
                      <span className="hidden md:inline-block">Amount</span>Earn
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr htmlFor="metal" className="transition duration-300 ease-in-out hover:bg-green-200 border-b bg-green-100 border-green-200">
                    ...
                    <tr htmlFor="plastic" className="transition duration-300 ease-in-out hover:bg-gray-200 border-b bg-gray-100 border-gray-200">
                      <td className="text-sm md:text-xl font-semiblod text-gray-900  px-6 py-4 whitespace-nowrap">
                        Plastic
                      </td>
                      <td className="text-sm md:text-xl font-light text-gray-900 px-6 py-4 whitespace-nowrap">
                    ...
                    <tr className="transition duration-300 ease-in-out hover:bg-gray-100 border-b bg-blue-100 border-blue-200">
                      <td className="px-6 py-4 whitespace-nowrap text-sm md:text-xl font-medium text-gray-900 cursor-pointer" onClick={calculateTotal}>
                        Total Waste</td>
                      ...
                      <td className={`text-sm md:text-xl font-bold text-gray-900 px-6 py-4 whitespace-nowrap text-center ${ total >= 0 ? "text-green-500 " : "text-red-500"}`}>
                        {total >= 0 ? (<p className=""> EARN : <span className="">{total}&#8358;</span> </p>
                        ) : (<p className=""> PAY : <span className="">{total.toString().substring(1)}&#8358;</span></p>)}
                      </td>
                    </tr>
                  </tbody>
                </table>
          ...
          <div className="hidden md:block">
            {payment ? 
            <div className={`${ total >= 0 ? "text-green-500 " : "text-red-500"} py-3 text-center text-lg font-semibold`}>
              {total >= 0 ? <>You Earn {total}&#8358; for your waste</> : <>You are to pay {total.toString().substring(1)}&#8358; for your waste</>}    
            </div>
            : ""}
          </div>
          {/* buttons */}
          <div className="p-5 w-full mx-auto flex justify-around">
            ...
            <button onClick={calculateTotal} className={`${payment ? "hidden" : ""} bg-blue-600 min-w-36 text-white font-bold py-3 px-10 hover:bg-blue-500 rounded-md`}>
              Calculate
            </button>
            <button onClick={clearValue} className={`${total >= 0  ? "" : "hidden"} bg-blue-600 min-w-36 text-white font-bold py-3 px-10 hover:bg-blue-500 rounded-md`}>
              Clear
            </button>
            {total >= 0 ? 
